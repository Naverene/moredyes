...
public class BlockPistonHead extends DirectionalBlock
{
    ...
    private static VoxelShape[] getShapesForExtension(boolean extended)
    {
        return Arrays.stream(Direction.values()).map((direction) ->
                getShapeForDirection(direction, extended)).toArray(VoxelShape[]::new);
    }
    ...
    public BlockPistonHead(AbstractBlock.Properties properties)
    {
        super(properties);
        this.setDefaultState(this.stateContainer.getBaseState().with(FACING, Direction.NORTH).with(TYPE, PistonType.DEFAULT).with(SHORT, false));
    }
